    ...
    if (event is TextChanged) {
      ...
      if (searchTerm.isEmpty) {
        ...
        try {
          ...
          Search_relsult result = new Search_relsult.fromJson(
      ...
      final String searchTerm = event.text;
      final int pageNo = event.pageNo;
      try {
        var response = await API.search(searchTerm);
        Search_relsult result =
            new Search_relsult.fromJson(json.decode(response.body.toString()));
        List<Pages> list = result.query.pages;
        yield SearchStateSuccess(list);
      } catch (error) {
        /* yield error is SearchResultError
